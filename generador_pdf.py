...
Espera recibir una lista de productos ya formateada por
motor_recomendacion.formatear_para_reporte(), es decir, cada producto
es un dict con las claves:
    producto, ingrediente_activo, dosis_por_ha,
    cantidad_total_despachar, momento_aplicacion, beneficio_principal
"""
...
def generar_pdf(datos_generales: dict, productos_seleccionados: list, ruta_salida: str):
    ...
    # ---- Tabla de productos ----
    col_widths = [26, 38, 22, 32, 34]
    headers = ["Producto", "Ingrediente activo", "Dosis/ha", "Cantidad Total\na Despachar", "Momento de aplicación"]
    ...
    for w, h in zip(col_widths, headers):
        ...
        pdf.multi_cell(w, 4, h, border=0, align="C", fill=True)
        x_header += w
    pdf.set_xy(10, y_header + 8)
    ...
    for prod in productos_seleccionados:
        fila = [
            ...
            prod.get("cantidad_total_despachar", "-"),
        ...
        for w, texto in zip(col_widths, fila):
            n_lineas = max(1, len(str(texto)) // 18 + 1)
    ...
    pdf.set_text_color(*GRIS_TEXTO)
    notas = (
        "- La columna 'Cantidad Total a Despachar' se calcula multiplicando la dosis por "
        "hectarea por el area total del lote registrada en este reporte.\n"
